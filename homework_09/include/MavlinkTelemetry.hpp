...
public:
    ...
    void triggerCargoDrop(float drop_x, float drop_y, float drop_z) {
        if (drop_acked_) return; // Вже успішно скинуто і підтверджено
...
private:
    ...
    void sendGlobalPositionAndAttitude(float x, float y, float z, float vx, float vy, float dir_rad, uint32_t t_ms) {
        ...
        mavlink_msg_attitude_pack(
            sysid_, compid_, &msg_att,
            t_ms,
            0.0f, 0.0f, dir_rad, // roll=0, pitch=0, yaw=dir_rad
            0.0f, 0.0f, 0.0f     // rates
        );
        sendBuffer(msg_att);
    }
    ...
    void sendDropCommandLong() {
        ...
        mavlink_msg_command_long_pack(
            ...
            0, // confirmation
            pending_drop_cmd_.param1, pending_drop_cmd_.param2,
            pending_drop_cmd_.param3, pending_drop_cmd_.param4,
            pending_drop_cmd_.param5, pending_drop_cmd_.param6,
            pending_drop_cmd_.param7
        );
        sendBuffer(msg);
        last_cmd_sent_time_ = std::chrono::steady_clock::now();
        drop_retries_left_--;
        std::cout << "[MAVLink] COMMAND_LONG (MAV_CMD_USER_1) sent. Retries left: " 
                  << drop_retries_left_ << std::endl;
    }

    void processDropRetries() {
        if (!drop_active_ || drop_acked_) return;
    ...
    void pollIncomingPackets() {
        ...
        while (true) {
            ...
            if (bytes_rx <= 0) break; // Даних у неблокуючому сокеті більше немає
